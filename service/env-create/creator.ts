...
import os from 'os';
import fs from 'fs';
import type { Dependency, Environment, EnvironmentType } from '../../src/types';
...
export class EnvironmentCreator {
  ...
  private async createPythonEnv(
    ...
    try {
      ...
      return {
        ...
        status: 'healthy',
        tags: useConda
          ? ['python', version, 'conda', 'managed']
          : ['python', version, 'venv', 'managed'],
        dependencies: [],
  ...
  private resolvePythonLauncher(version: string): string {
    const normalized = version === 'latest' ? '' : (version.match(/\d+\.\d+/)?.[0] ?? '');
    const candidates = [normalized ? `python${normalized}` : '', 'python3', 'python'].filter(
      Boolean
    );

    for (const candidate of candidates) {
      try {
        const lookupCommand = this.isWindows ? 'where.exe' : 'command -v';
        const output = execSync(`${lookupCommand} ${candidate}`, {
          encoding: 'utf-8',
          stdio: 'pipe',
        })
          .split(/\r?\n/)
          .map((entry) => entry.trim())
          .find((entry) => entry && !entry.toLowerCase().includes('windowsapps'));
  ...
  private readInstalledDependencies(type: EnvironmentType, envDir: string): Dependency[] {
    try {
      if (type === 'python') {
        const pythonExe = this.isWindows
          ? path.join(envDir, 'Scripts', 'python.exe')
   ...
   * 创建 Node.js 环境
   */
  private async createNodeEnv(name: string, version: string, envDir: string): Promise<Environment> {
    try {
      // 创建 Node.js 环境目录
      fs.mkdirSync(envDir, { recursive: true });
      ...
      fs.writeFileSync(path.join(envDir, 'package.json'), JSON.stringify(packageJson, null, 2));
   ...
   * 创建 Java 环境
   */
  private async createJavaEnv(name: string, version: string, envDir: string): Promise<Environment> {
    try {
      fs.mkdirSync(envDir, { recursive: true });
   ...
   * 创建 Go 环境
   */
  private async createGoEnv(name: string, version: string, envDir: string): Promise<Environment> {
    try {
      fs.mkdirSync(envDir, { recursive: true });
